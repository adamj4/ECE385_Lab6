...
module  color_mapper ( input  logic [9:0] drawX, drawY,
    ...
    always_comb begin
        logic [6:0] col = drawX / TEXT_WIDTH;
        logic [4:0] row = drawY / TEXT_HEIGHT;
        reg_to_read = (row * NUM_COLS + col) / 4;
        char_to_read = (row * NUM_COLS + col) % 4;
        
        char_byte = reg_value[char_to_read * 8 +: 8];
        addr = char_byte[6:0] * 16 + (drawY - row * TEXT_HEIGHT);
        
        pixel_val = data[7 - (drawX - col * TEXT_WIDTH)];
        if (char_byte[7])
            pixel_val = ~pixel_val;
